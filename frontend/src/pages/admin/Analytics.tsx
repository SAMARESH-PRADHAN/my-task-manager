...
import { format, subDays, subWeeks, startOfDay, endOfDay, isWithinInterval, startOfWeek, endOfWeek, subMonths, startOfMonth, endOfMonth } from 'date-fns';
...
const Analytics: React.FC = () => {
  ...
  const serviceDistribution = useMemo(() => {
    const jobSeekerCount = filteredData.formFilling.filter((t) => t.serviceType === 'job_seeker').length;
    const studentCount = filteredData.formFilling.filter((t) => t.serviceType === 'student').length;
    const govSchemeCount = filteredData.formFilling.filter((t) => t.serviceType === 'gov_scheme').length;
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-6">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <Card className="shadow-card">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold text-foreground">{filteredData.formFilling.length}</p>
          </CardContent>
        </Card>
        <Card className="shadow-card">
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Xerox/Other</p>
            <p className="text-3xl font-bold text-foreground">{filteredData.xerox.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="shadow-card">
          <CardHeader>
            <CardTitle className="text-lg">Revenue Analysis by Service Type</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={employeePerformance} layout="vertical">
                  ...
                  <Tooltip
                    formatter={(value, name) => [`₹${value}`, name === 'formFilling' ? 'Form Filling' : 'Xerox/Other']}
                    contentStyle={{
                      backgroundColor: 'hsl(var(--popover))',
                      border: '1px solid hsl(var(--border))',
                      borderRadius: '8px',
                  ...
                  />
                  <Legend />
                  <Bar dataKey="formFilling" name="Form Filling" stackId="a" fill="hsl(var(--chart-1))" />
                  <Bar dataKey="xerox" name="Xerox/Other" stackId="a" fill="hsl(var(--chart-2))" />
                </BarChart>
              </ResponsiveContainer>
            </div>
