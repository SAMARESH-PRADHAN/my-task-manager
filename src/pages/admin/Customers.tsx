...
const Customers: React.FC = () => {
  ...
  const handleDownload = () => {
    const data = filteredCustomers.map((customer, index) => ({
      ...
      'Created Date': format(new Date(customer.createdAt), 'dd/MM/yyyy HH:mm'),
    }));
  ...
  const handleNotifyAll = () => {
    ...
    toast.success(`Message sent to ${filteredCustomers.length} customers!`);
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'job_seeker', 'student', 'gov_scheme'] as CustomerTab[]).map((tab) => (
          <Button
            key={tab}
                ...
                {paginatedCustomers.length === 0 ? (
                  ...
                  paginatedCustomers.map((customer, index) => (
                    <TableRow key={customer.id}>
                      ...
                      <TableCell className="capitalize">{customer.type.replace('_', ' ')}</TableCell>
                      <TableCell>
                        <div>
                          <p>{format(new Date(customer.createdAt), 'dd/MM/yyyy')}</p>
        ...
        <DialogContent className="bg-card border border-border">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4 justify-end">
              ...
              <Button onClick={handleNotifyAll} className="gradient-primary text-primary-foreground">
                Send
              </Button>
            </div>
          </div>
